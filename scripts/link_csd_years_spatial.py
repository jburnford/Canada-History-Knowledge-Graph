...
def load_year_layer(gdb_path: str, year: int) -> gpd.GeoDataFrame:
    ...
    if gdf.crs.to_epsg() != 3347:
        print(f"  Reprojecting from {gdf.crs.to_epsg()} to EPSG:3347...", file=sys.stderr)
...
def link_year_pair(
    ...
    # Spatial join to find potential overlaps
    print("  Computing spatial joins...", file=sys.stderr)
    # Reset index on gdf_to to ensure we can join properly
    gdf_to_indexed = gdf_to.reset_index(drop=True)
    joined = gpd.sjoin(gdf_from, gdf_to_indexed, how='inner', predicate='intersects', lsuffix='_from', rsuffix='_to')

    print(f"  Found {len(joined)} potential overlaps", file=sys.stderr)
    ...
    for idx, row in joined.iterrows():
        # Get the index of the matched CSD in gdf_to
        # GeoPandas sjoin creates 'index_right' column with the right table's index
        if 'index_right' in row:
            to_idx = row['index_right']
        elif 'index_to' in row:
            to_idx = row['index_to']
        else:
            # Fallback: row already has _to columns from the join
            # This happens in newer GeoPandas versions
            geom_from = row['geometry']
            geom_to = row.get('geometry_to', row['geometry'])  # Might be geometry or geometry_to
            area_from = row['area_from']
            area_to = row.get('area_to', row.get('area'))

            # Compute spatial overlap
            iou, frac_from, frac_to = analyze_overlap(geom_from, geom_to, area_from, area_to)
            ...
            # Build link record
            link = {
                f'tcpuid_{year_from}': row['tcpuid_from'],
                f'csd_name_{year_from}': row['csd_name_from'],
                f'cd_name_{year_from}': row['cd_name_from'],
                f'pr_{year_from}': row['pr_from'],
                f'tcpuid_{year_to}': row['tcpuid_to'],
                f'csd_name_{year_to}': row['csd_name_to'],
                f'cd_name_{year_to}': row['cd_name_to'],
                f'pr_{year_to}': row['pr_to'],
        ...
        # Get geometries and areas using index
        geom_from = row['geometry']
        geom_to = gdf_to_indexed.loc[to_idx, 'geometry']
        area_from = row['area_from']
        area_to = gdf_to_indexed.loc[to_idx, 'area']
        ...
        name_sim = compute_name_similarity(
            row['csd_name_from'],
            gdf_to_indexed.loc[to_idx, 'csd_name'],
            row['cd_name_from'],
            gdf_to_indexed.loc[to_idx, 'cd_name']
        )
        ...
        link = {
            ...
            f'cd_name_{year_from}': row['cd_name_from'],
            f'pr_{year_from}': row['pr_from'],
            f'tcpuid_{year_to}': gdf_to_indexed.loc[to_idx, 'tcpuid'],
            f'csd_name_{year_to}': gdf_to_indexed.loc[to_idx, 'csd_name'],
            f'cd_name_{year_to}': gdf_to_indexed.loc[to_idx, 'cd_name'],
            f'pr_{year_to}': gdf_to_indexed.loc[to_idx, 'pr'],
            'relationship': rel_type,
            'iou': round(iou, 4),
            'frac_from': round(frac_from, 4),
            'frac_to': round(frac_to, 4),
