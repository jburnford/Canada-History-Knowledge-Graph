#!/usr/bin/env python3
"""
Generate P132_spatiotemporally_overlaps_with relationships from temporal link data.

Converts CSD temporal links into CIDOC-CRM P132_spatiotemporally_overlaps_with
relationships for Neo4j import. This models overlap between spatiotemporal
volumes of places across census years.
...
def load_csd_links(links_dir: Path) -> pd.DataFrame:
    ...
    for year_from, year_to in year_pairs:
        ...
        if link_file.exists():
            ...
            # Filter for high-confidence links suitable for P132
            ...
            # Create P132 relationships (column names are year-specific)
...
def main():
    parser = argparse.ArgumentParser(description='Generate P132_spatiotemporally_overlaps_with relationships')
    ...
    print(f"\n{'='*60}", file=sys.stderr)
    print(f"Generating P132_spatiotemporally_overlaps_with relationships", file=sys.stderr)
    ...
    if len(csd_links) > 0:
        csd_file = out_dir / 'p132_spatiotemporally_overlaps_with_csd.csv'
        csd_links.to_csv(csd_file, index=False)
        print(f"\n✓ CSD P132 relationships: {len(csd_links)} → {csd_file}", file=sys.stderr)

        # Summary by relationship type
        print(f"\n  CSD P132 by type:", file=sys.stderr)
        for rel_type in ['SAME_AS', 'CONTAINS', 'WITHIN']:
            count = len(csd_links[csd_links['overlap_type'] == rel_type])
            if count > 0:
    ...
    print(f"{'='*60}", file=sys.stderr)
    print(f"Total P132_spatiotemporally_overlaps_with relationships: {len(csd_links)}", file=sys.stderr)
