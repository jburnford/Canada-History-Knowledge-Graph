#!/usr/bin/env python3
"""
Import 1921 census data to Neo4j Canada Census database.
"""
...
def import_1921_data():
    ...
    try:
        with driver.session() as session:
            # Import E93_Presence (CSD) nodes for 1921
            print("Importing E93_Presence (CSD) nodes for 1921...")
            df = pd.read_csv(DATA_DIR / 'e93_presence_1921.csv')
            ...
            # Import E94_Space_Primitive nodes for 1921
            print("Importing E94_Space_Primitive nodes for 1921...")
            df = pd.read_csv(DATA_DIR / 'e94_space_primitive_1921.csv')
            for _, row in df.iterrows():
                session.run("""
            ...
            df = pd.read_csv(DATA_DIR / 'p166_was_presence_of_1921.csv')
            for _, row in df.iterrows():
                session.run("""
            ...
            df = pd.read_csv(DATA_DIR / 'p164_temporally_specified_by_1921.csv')
            for _, row in df.iterrows():
                session.run("""
                    MATCH (presence:E93_Presence {presence_id: $start_id})
            ...
            df = pd.read_csv(DATA_DIR / 'p161_spatial_projection_1921.csv')
            for _, row in df.iterrows():
                session.run("""
                    MATCH (presence:E93_Presence {presence_id: $start_id})
            ...
            df = pd.read_csv(DATA_DIR / 'p89_falls_within_1921.csv')
            for _, row in df.iterrows():
                session.run("""
            ...
            df = pd.read_csv(DATA_DIR / 'p122_borders_with_1921.csv')
            for _, row in df.iterrows():
                session.run("""
                ...
                """, start_id=row[':START_ID'] + '_1921', end_id=row[':END_ID'] + '_1921',
                     border_length=float(row['shared_border_length_m:float']),
                     during_period=row['during_period'])
            print(f"  ✓ Created {len(df)} P122 relationships")

            # Import CD presences
            print("Importing E93_Presence (CD) nodes for 1921...")
            df = pd.read_csv(DATA_DIR / 'e93_presence_cd_1921.csv')
            for _, row in df.iterrows():
                session.run("""
                    CREATE (p:E93_Presence {
                        presence_id: $presence_id,
            ...
            print("Importing E94_Space_Primitive (CD) nodes for 1921...")
            df = pd.read_csv(DATA_DIR / 'e94_space_primitive_cd_1921.csv')
            for _, row in df.iterrows():
                session.run("""
                    CREATE (s:E94_Space_Primitive {
                        space_id: $space_id,
            ...
            df = pd.read_csv(DATA_DIR / 'p166_was_presence_of_cd_1921.csv')
            for _, row in df.iterrows():
                session.run("""
                    MATCH (presence:E93_Presence {presence_id: $start_id})
                    MATCH (place:E53_Place {place_id: $end_id})
            ...
            df = pd.read_csv(DATA_DIR / 'p164_temporally_specified_by_cd_1921.csv')
            for _, row in df.iterrows():
                session.run("""
                    MATCH (presence:E93_Presence {presence_id: $start_id})
                    MATCH (period:E4_Period {period_id: $end_id})
            ...
            df = pd.read_csv(DATA_DIR / 'p161_spatial_projection_cd_1921.csv')
            for _, row in df.iterrows():
                session.run("""
                    MATCH (presence:E93_Presence {presence_id: $start_id})
                    MATCH (space:E94_Space_Primitive {space_id: $end_id})
            ...
            df = pd.read_csv(DATA_DIR / 'p10_csd_within_cd_presence_1921.csv')
            for _, row in df.iterrows():
                session.run("""
                    MATCH (csd:E93_Presence {presence_id: $start_id})
