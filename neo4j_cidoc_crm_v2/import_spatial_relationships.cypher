// Canadian Census CIDOC-CRM Spatial Relationships Import Script (v2 - Persistent Places)
// Part 2: Relationship Import
// Changes: P166/P89/P122 use persistent_place_id, added lineage + E42 provenance

// ============================================================
// STEP 1: Import P166_was_a_presence_of (CSD presence → persistent CSD place)
// ============================================================
...
// ============================================================
// STEP 7: Import P89_falls_within (persistent CSD place → CD place)
// ============================================================
UNWIND [1851, 1861, 1871, 1881, 1891, 1901, 1911, 1921] AS year
CALL {
  WITH year
...
// ============================================================
// STEP 9: Import P122_borders_with (persistent CSD place → persistent CSD place)
// ============================================================
UNWIND [1851, 1861, 1871, 1881, 1891, 1901, 1911, 1921] AS year
CALL {
  WITH year
...
// ============================================================
// STEP 12: Import place lineage (SPLIT_FROM / MERGED_INTO)
// ============================================================
LOAD CSV WITH HEADERS FROM 'file:///place_lineage.csv' AS row
CALL {
  WITH row
  MATCH (place1:E53_Place {place_id: row.`:START_ID`})
  MATCH (place2:E53_Place {place_id: row.`:END_ID`})
  CREATE (place1)-[r:PLACE_LINEAGE {
    lineage_type: row.lineage_type,
    change_year: toInteger(row.`change_year:int`)
  }]->(place2)
} IN TRANSACTIONS OF 500 ROWS;

// ============================================================
// STEP 13: Import E42_Identifier provenance (presence → TCP UID)
// ============================================================
LOAD CSV WITH HEADERS FROM 'file:///p1_identifier_provenance.csv' AS row
CALL {
  WITH row
  MATCH (presence:E93_Presence {presence_id: row.`:START_ID`})
  MATCH (identifier:E42_Identifier {identifier_id: row.`:END_ID`})
  CREATE (presence)-[:P1_is_identified_by {type: row.type}]->(identifier)
} IN TRANSACTIONS OF 1000 ROWS;

// ============================================================
// STEP 14: Import E41_Appellation nodes
// ============================================================
...
// ============================================================
// STEP 15: Import P1_is_identified_by (canonical: persistent place → appellation)
// ============================================================
LOAD CSV WITH HEADERS FROM 'file:///p1_is_identified_by.csv' AS row
CALL {
  WITH row
  WITH row WHERE row.type = 'canonical_name'
...
// ============================================================
// STEP 16: Import P1_is_identified_by (variant: presence → appellation)
// ============================================================
LOAD CSV WITH HEADERS FROM 'file:///p1_is_identified_by.csv' AS row
CALL {
  WITH row
  WITH row WHERE row.type = 'variant_name'
  MATCH (presence:E93_Presence {presence_id: row.`:START_ID`})
  MATCH (appellation:E41_Appellation {appellation_id: row.`:END_ID`})
